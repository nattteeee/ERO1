...
def from_graph_to_matrix(G):
    l = len(G.degree)
    duo = associate_node_to_matrix(G, l)
    res = [[0 for k in range(l)] for i in range(l)]
    for data in G.edges(data = True):
        i, j = search_index(duo, data[0]), search_index(duo, data[1])
        res[i][j], res[j][i] = data[2]["length"], data[2]["length"]
    return res, duo


def associate_node_to_matrix(G, l):
    list = [0 for a in range(l)]
    for k in range(l):
        list[k] = [k]
    i = 0
    for data in G.nodes:
